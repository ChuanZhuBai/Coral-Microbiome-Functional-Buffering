...
keep <- filterByExpr(
  ...
  min.count = 1,
  min.total.count = 3,  # 因样本量较少，放宽过滤阈值
  large.n = 1,
  min.prop = 0.05
)
...
# ==================== 2. 数据加载与预处理 ====================
# 读取原始counts数据
counts_data <- read.csv("KEGG_bases _ IT-C VS IT-H.csv", header = TRUE, row.names = 1, check.names = FALSE)
...
dge <- DGEList(
  counts = counts_data,
  group = factor(metadata$Group, levels = c("IT_C", "IT_H")),  # ST为参照
...
contrast <- makeContrasts(IT_H - IT_C, levels = design)
...
results <- results %>%
  mutate(
    Significance = case_when(
      PValue < 0.05 & logFC > 1 ~ "IT-H UP",
      PValue < 0.05 & logFC < -1 ~ "IT-C UP",
...
label_data <- rbind(it_top5, st_top5) %>%
  mutate(
    ...
    Direction = ifelse(logFC > 0, "IT_H", "IT_C")
  )

# ==================== 9. 设置颜色方案 ====================
colors <- c(
  "IT-H UP" = "#E64B35",  # 深红色"IT-H" = "#E64B35", "ST-H" = "#4DBBD5"
  "IT-C UP" = "#4DBBD5",  # 深蓝色
...
volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(PValue))) +
  ...
  scale_color_manual(values = c("IT_H" = "#D53E4F", "IT_C" = "#3288BD")) +
  
  # 坐标轴和标题
  labs(
    x = expression(log[2]("Fold Change (IT-H/IT-C)")),
    y = expression(-log[10]("FDR")),
    title = "Differential KEGG Pathway Abundance",
    subtitle = sprintf("IT-H (n=%d) vs IT-C (n=%d) | Top 5 most significant KOs labeled", 
                       sum(metadata$Group == "IT_H"), 
                       sum(metadata$Group == "IT_C")),
    fill = "Significance"
  ) +
...
write.csv(
  ...
  "KEGG_diff_results_IT-H_vs_IT-C.csv",
  row.names = FALSE
)
# 保存分析结果
write.csv(
  results[order(results$FDR), ],
  "KEGG_diff_results_IT-H_vs_IT-C-1.csv",
  row.names = FALSE
)
# 打印统计摘要
...
cat("IT-H UP KOs (PValue<0.05 & LFC>1.5):", sum(results$PValue < 0.05 & results$logFC > 1), "\n")
cat("IT-C UP KOs (PValue<0.05 & LFC<-1.5):", sum(results$PValue < 0.05 & results$logFC < -1), "\n")
