##############################################
# 潮间带vs潮下带实验室对照组(ST-H vs ST-C) KEGG通路差异火山图
# 最终优化版本 - 仅保留基础差异分析
############################################################################
##############################################
# 潮间带vs潮下带实验室对照组(IT-C vs IT-H) KEGG通路差异火山图
# 最终优化版本 - 仅保留基础差异分析
############################################################################
...
# ==================== 2. 数据加载与预处理 ====================
# 读取原始counts数据
counts_data <- read.csv("KEGG_bases_ST-C VS ST-H.csv", header = TRUE, row.names = 1, check.names = FALSE)
...
dge <- DGEList(
  counts = counts_data,
  group = factor(metadata$Group, levels = c("ST_C", "ST_H")),  # ST为参照
...
contrast <- makeContrasts(ST_H - ST_C, levels = design)
...
results <- results %>%
  mutate(
    Significance = case_when(
      PValue < 0.05 & logFC > 1 ~ "ST-H UP",
      PValue < 0.05 & logFC < -1 ~ "ST-C UP",
...
label_data <- rbind(it_top5, st_top5) %>%
  mutate(
    ...
    Direction = ifelse(logFC > 0, "ST_H", "ST_C")
  )

# ==================== 9. 设置颜色方案 ====================
colors <- c(
  "ST-H UP" = "#E64B35",  # 深红色"#E64B35", "ST-H" = "#4DBBD5"
  "ST-C UP" = "#4DBBD5",  # 深蓝色
...
volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(PValue))) +
  ...
  scale_color_manual(values = c("ST_H" = "#D53E4F", "ST_C" = "#3288BD")) +
  
  # 坐标轴和标题
  labs(
    x = expression(log[2]("Fold Change (ST-H/ST-C)")),
    y = expression(-log[10]("FDR")),
    title = "Differential KEGG Pathway Abundance",
    subtitle = sprintf("ST-H (n=%d) vs ST-C (n=%d) | Top 5 most significant KOs labeled", 
                       sum(metadata$Group == "ST_H"), 
                       sum(metadata$Group == "ST_C")),
    fill = "Significance"
  ) +
...
write.csv(
  ...
  "KEGG_diff_results_ST-H_vs_ST-C.csv",
  row.names = FALSE
)

write.csv(
  results[order(results$FDR), ],
  "KEGG_diff_results_ST-H_vs_ST-C-1.csv",
  row.names = FALSE
)
# 打印统计摘要
...
cat("ST-H UP KOs (PValue<0.05 & LFC>1.5):", sum(results$PValue < 0.05 & results$logFC > 1), "\n")
cat("ST-C UP KOs (PValue<0.05 & LFC<-1.5):", sum(results$PValue < 0.05 & results$logFC < -1), "\n")
